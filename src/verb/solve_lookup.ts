...
for (var index = 0; index < regular.length; index++) {
	const present = regular[index];
	const past = solveFromRegexes(present, "VBD");
	const pastParticiple = solveFromRegexes(present, "VBN");
	const present3rd = solveFromRegexes(present, "VBZ");
	const gerund = solveFromRegexes(present, "VBG");
	const entry = [present, past, pastParticiple, present3rd, gerund];
	VBP[present] = entry;
	VBD[past] = entry;
